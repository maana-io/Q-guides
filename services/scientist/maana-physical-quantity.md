...
| Query | { unitOfMeasure(unitRef:{name:"mi/hr"}) {  aliases}} |
...
| Query | { unitOfMeasure(unitRef:{name:"N"}){dimension{name}}} |
...
| Query | query { physicalDimensions {  name alias formula}} |
...
| Query | query {  systemOfUnits\(systemRef:{name:"Imperial"}\){  units{ name}}} |
...
| Query | query IMPERIAL{systemOfUnits\(systemRef:{name:"Imperial"}\){ ... SystemOfUnitsFragment}}<br>query EXAMPLE\($IMPERIAL:SystemOfUnitsInput\){ parse\(source:"400 psi",systemInputs:\[$IMPERIAL\]\){  name}} |
| Response | { "data": { "parse": { "name": "400.0 lb/in²"}}} |
...
| Query | query PRESSURE{physicalDimension\(dimensionRef:{name:"PRESSURE"}\){ ... PhysicalDimensionFragment }}<br>query EXAMPLE($PRESSURE:PhysicalDimensionInput){ parse\(source:"400 meters",dimensionInputs:[$PRESSURE]\){  name }} |
| Response | { "data": { "parse": { "name": "400.0 lb/in²"}}} |
...
| Query | extractBatch\(sources: \[ "the airspeed of the plane was 350 mph as it passed through 3000 meters.", "The minimum runway length is 7500 feet."\]\) {  entity {  name}} |
...
| Query | query IMPERIAL{systemOfUnits\(systemRef:{name:"Imperial"}\){ ... SystemOfUnitsFragment}}<br>query EXAMPLE\($IMPERIAL:SystemOfUnitsInput\){ extractBatch\(sources: \["the airspeed of the plane was 350 mph as it passed through 3000 meters.","The minimum runway length is 7500 feet."\], systemInputs:\[$IMPERIAL\]\) {  entity {  name}}} |
...
| Query | { extractBatch\(sources: \[ "the airspeed of the plane was 350 mph as it passed through 3000 meters.", "The minimum runway length is 7500 feet."\], dimensionInputs:\[{name:"LENGTH",formula:"L¹"}\]\) {  entity {  name}}} |
...
| Query | query LENGTH{physicalDimension\(dimensionRef:{name:"PRESSURE"}\){ ... PhysicalDimensionFragment }}<br>query EXAMPLE($PRESSURE:PhysicalDimensionInput){ extractBatch\(sources: \[ "the airspeed of the plane was 350 mph as it passed through 3000 meters.", "The minimum runway length is 7500 feet."\], dimensionInputs:\[$LENGTH\] \) {  entity {  name} fromText, fromSpan, fromOffset}} |
...
| using nested calls | query TOUNIT{ unitOfMeasure(unitRef:{name:"lb/in²"}){ ...UnitOfMeasureFragment }}<br>query EXAMPLE($TOUNIT:UnitOfMeasureInput!){parse(source:"100 mbar"){ convertTo(toUnitInput:$TOUNIT){ roundTo(precision:2){name}}}} |
| using arguments | query TOUNIT{ unitOfMeasure(unitRef:{name:"lb/in²"}){ ...UnitOfMeasureFragment }}<br>query EXAMPLE($TOUNIT:UnitOfMeasureInput!){convertTo(input:{parseFrom:"100 mbar"},toUnitInput:$TOUNIT,precision:2){ name}} |
...
| Query | query TOUNIT{ unitOfMeasure(unitRef:{name:"lb/in²"}){ ...UnitOfMeasureFragment }}<br>query EXAMPLE($TOUNIT:UnitOfMeasureInput!){canBeConvertedTo(input:{parseFrom:"100 mbar"}, toUnitInput:$TOUNIT)} |
...
| Query | { product\(factors: \[{parseFrom: "12 inch"}, {parseFrom: "1 Hz"}, {parseFrom: "1/3 acre"}\], precision: 2\) {  name,  unitOfMeasure {  dimension {  formula}}}} |
...
| Query | query REFERENCE{ parse(source:"35 cm") { ... PhysicalQuantityFragment }}<br>query EXAMPLE($REF:PhysicalQuantityInput!){extractBatch(sources: ["1 inch","1 foot","1 yard"]) { entity{  isGT(rhs:$REFERENCE)}}} |
